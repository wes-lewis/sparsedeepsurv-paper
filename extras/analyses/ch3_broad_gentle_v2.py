#!/usr/bin/env python3
"""
Broad gated sweep under the gentler baseline-like regime (v2).

2026-09-13 fix, motivated by a completeness gap in the chapter-3
manuscript: the primary broad sweep (ch3_kipan_broad.py /
ch3_brca_broad_v2.py) only covers the MLP-predictor families (LSPIN,
Concrete); the manuscript's own Figure 4 caption confirms this
("with LSPIN and Concrete parameterizations shown separately"). The
linear-predictor families (L-LSPIN, L-Concrete) that the chapter's own
conclusion recommends -- especially for BRCA -- were only attempted in
this "gentle" script's fragmented, never-consolidated history
(COLLABORATOR_GUIDE.md: "Replication attempts... have produced mixed
results so far").

v1 of this script swept `sigma` for every family, including Concrete and
L-Concrete, and never swept `temperature` for any family. That is a real
bug, not just an incomplete sweep: `gate_sigma` is a confirmed no-op for
Concrete-type gates (deepsurv_gated.py's `_sample_concrete` only ever
reads `self.temperature`), so the entire Concrete/L-Concrete `sigmas`
dimension in v1 was wasted compute producing duplicate results, and
their real lever (temperature) was never touched at all. Symmetrically,
`temperature` is a no-op for LSPIN-type gates (`_sample_lspin_tf` never
references it), so LSPIN/L-LSPIN's existing `sigmas` sweep was already
the correct lever and needs no change.

v2 fixes this: LSPIN-type families sweep `sigmas` (their lever) with
`temperature` fixed at a single value; Concrete-type families sweep
`temperatures` (their lever, newly added) with `gate_sigma` fixed at a
single value instead of wastefully duplicated. This mirrors the
per-family-correct-lever approach validated on the synthetic ground-
truth-recovery task (POST_DISSERTATION_PLAN.md thread 5), which turned a
similarly confounded, inconclusive sweep into a statistically
significant result once each family was tuned with its own real lever.

Retains the rest of v1's design:
...
DATASET_SPECS = {
    "kipan": {
        ...
        "plot_x_max": 3000.0,
        "families": {
            # LSPIN-type gates: gate_sigma is the real lever (temperature is a
            # no-op for _sample_lspin_tf), so sigmas is the swept dimension
            # and temperature is fixed at a single value.
            "LSPIN": {
                "gate_type": "lspin_tf",
                "predictor": "mlp",
                "sigmas": [0.15, 0.25],
            ...
            # Concrete-type gates: temperature is the real lever (gate_sigma
...
def _build_configs(args) -> List[Dict]:
    ...
    for family_label, family in spec["families"].items():
        # Both dimensions are looped generically, but per-family only one is
        # ever a real multi-value sweep (the other is a single-element list)
        # -- see the DATASET_SPECS comment for why (gate_sigma is a no-op for
        # Concrete-type gates, temperature is a no-op for LSPIN-type gates).
        for sigma in family["sigmas"]:
            for temp in family["temperatures"]:
...
def _parse() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help="Restrict to these families (default: all four). Useful for a cheap pilot.",
    )
    p.add_argument(
        "--pilot", action="store_true",
        help=(
            "Shrink the sweep to a cheap exploratory pass: only the first/"
            "middle/last lambda per family, smooth=0 only (skip the "
            "smoothing dimension), and widen the sigma/temperature test "
            "values beyond the v1 defaults (which were never validated for "
            "the linear-predictor families, and for Concrete-type families "
            "used the wrong parameter entirely -- see module docstring). "
            "Use this to find reasonable operating ranges before committing "
            "to the full n-reps x full-grid redo."
        ),
    )
    p.add_argument(
        "--pilot-sigmas", type=float, nargs="+", default=[0.05, 0.1, 0.2, 0.4, 0.8],
        help="Sigma values to test in pilot mode, for LSPIN-type families.",
    )
    p.add_argument(
        "--pilot-temperatures", type=float, nargs="+", default=[0.1, 0.2, 0.4, 0.8, 1.6],
        help="Temperature values to test in pilot mode, for Concrete-type families.",
    )
    args = p.parse_args()
    spec = DATASET_SPECS[args.dataset]
    if args.families:
        spec = dict(spec)
        spec["families"] = {k: v for k, v in spec["families"].items() if k in args.families}
        DATASET_SPECS[args.dataset] = spec
    if args.pilot:
        spec = DATASET_SPECS[args.dataset]
        for fam_label, fam in spec["families"].items():
            fam = dict(fam)
            lambdas = fam["lambdas"]
            fam["lambdas"] = [lambdas[0], lambdas[len(lambdas) // 2], lambdas[-1]]
            if fam["gate_type"] == "lspin_tf":
                fam["sigmas"] = list(args.pilot_sigmas)
            else:
                fam["temperatures"] = list(args.pilot_temperatures)
            spec["families"][fam_label] = fam
        spec["sample_smooth_grid"] = [0.0]
