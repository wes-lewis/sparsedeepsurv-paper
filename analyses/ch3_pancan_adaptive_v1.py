...
Grid (PanCan adaptive v1, 80 configs total):
...
Usage:
  ...
  python analyses/ch3_pancan_adaptive_v1.py [--gpus 0 2 6 7]
"""
...
RESULTS_DEFAULT     = PAPER_ROOT / "data" / "runs" / "ch3_pancan_adaptive_v1"
...
def _parse():
    p = argparse.ArgumentParser(
        description="TCGA Pan-Cancer adaptive sweep v1: 3-rep phase 1 → select promising → full phase 2"
    )
    ...
    # Lambda grids (PanCan adaptive v1)
    ...
    p.add_argument("--concrete-lambdas", type=float, nargs="+",
                   default=[0.002, 0.0017, 0.0014, 0.0012, 0.0010])

    # Two sigmas per family (new for PanCan)
