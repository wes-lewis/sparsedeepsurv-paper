...
Mirrors ch3_kipan_broad.py structure but with BRCA-specific parameters:
  ...
  - sigmas:   LSPIN 0.10, Concrete 0.10
  - lambdas:  wide range calibrated to Khard ~200-900 for BRCA
  - smooth:   positive grid [0.1, 0.2, 0.4] (BRCA uses higher smoothing)
  - concrete_mode: ste (Straight-Through Estimator)
  - family labels: "LSPIN" and "Concrete"

Total configs: 2 × (12 nosmooth + 3 smooth × 8 smooth_lams) = 72 configs
With 10 reps/config = 720 total model trains.
...
def _post_process(results_dir: Path, args) -> None:
    ...
    merged.to_csv(results_dir / "brca_broad_tuning_merged_summary.csv", index=False)

    top = rank_top_configs(merged, x_min=float(args.plot_x_min), x_max=float(args.plot_x_max))
    top.to_csv(results_dir / "brca_broad_tuning_top_configs_in_band.csv", index=False)
    ...
    for family in ["LSPIN", "Concrete"]:
        for y_col, y_label in [
            ...
            save_family_loess(
                ...
                title=f"BRCA {family} broad: {y_label} vs sparsity",
...
def _parse():
    p = argparse.ArgumentParser(description="BRCA broad sweep (paper repo, knn_k=5, multi-GPU, STE concrete)")
    ...
    # Gate params — BRCA calibrated to lower sigma
    p.add_argument("--lspin-gate-sigma",    type=float, default=0.10)
    p.add_argument("--lspin-temperature",   type=float, default=0.5)
    p.add_argument("--concrete-gate-sigma", type=float, default=0.10)
    p.add_argument("--concrete-temperature",type=float, default=0.3)
    p.add_argument("--concrete-mode", choices=["relaxed", "ste"], default="ste")
    # Lambda grids — calibrated for BRCA Khard ~200-900
    p.add_argument("--positive-smooth-grid", type=float, nargs="+",
                   default=[0.1, 0.2, 0.4])
    p.add_argument("--lspin-nosmooth-lambdas", type=float, nargs="+",
                   default=[0.020, 0.015, 0.012, 0.010, 0.007, 0.005,
                             0.0033, 0.0025, 0.0018, 0.0013, 0.0009, 0.0006])
    p.add_argument("--lspin-smooth-lambdas", type=float, nargs="+",
                   default=[0.012, 0.009, 0.007, 0.005, 0.0033, 0.0025, 0.0018, 0.0013])
    p.add_argument("--concrete-nosmooth-lambdas", type=float, nargs="+",
                   default=[0.008, 0.006, 0.005, 0.004, 0.003, 0.0022,
                             0.0018, 0.0015, 0.0012, 0.0009, 0.0007, 0.0005])
    p.add_argument("--concrete-smooth-lambdas", type=float, nargs="+",
                   default=[0.006, 0.005, 0.004, 0.003, 0.0022, 0.0018, 0.0015, 0.0012])
    # Eval params
    ...
    # Plot params
    p.add_argument("--plot-x-min",  type=float, default=50.0)
    p.add_argument("--plot-x-max",  type=float, default=5000.0)
...
if __name__ == "__main__":
    ...
    # Build the full ordered config list
    families_spec = [
    ...
    print(f"[ch3_brca_broad_v2] {len(all_configs)} configs → {n_workers} workers on GPUs {gpus}")
